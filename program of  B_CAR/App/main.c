//作者：
//说明：saomiao文件下有一个lkcongzhongjiansaomiao（）函数是图像处理函数，偏差处理以及舵机控制均在此文件下
...
extern int podao_flag,zhangai_right,zhangai_left,qvlv_quanju,duandian,mubiao_speed,dawan_speed,qulv_jinduan,qulv_yuandaun;
extern void  lukuangudge();
char go=0,S1,S,zhichong_flag;
extern char lcd_show_enable1;
extern char jiji,papa;
extern int ruyuanhuan_flag;
//s16 speed_a;
...
void  main(void)
{
    ...
    dianji_canshu_init();      //电机参数初始化

    Priority_Set();           //中断优先级设置
    camera_init(imgbuff);                                  //这里设定  imgbuff 为采集缓冲区！！！！！！

    
    ftm_pwm_init(FTM2,FTM_CH0,10000,0);  //电机初始化  FTM2
    ftm_pwm_init(FTM2,FTM_CH1,10000,0);
    
    
    ftm_pwm_init(FTM0,FTM_CH6,300,4220);//舵机初始化  FTM0
       
    ftm_quad_init(FTM1);//编码器初始化    FTM1

   
    pit_init_ms(PIT0,5);//定时器中断5ms  控制时间是5ms一次
...
void PIT0_IRQHandler(void)//定时器中断服务函数
{
      //lcd_show_enable1=0;//注释掉是调用液晶
        ...
        if(lcd_show_enable1)//为0跳出液晶，执行扫描
        {
            my_lcd_show();
        }
        else
        {
                  ...
                  get_maichong();//获取电机转速 
                    ...
                    if(LK_jishi>=2000)    //延迟2000*5ms后（10s）检测起跑线
                        {
                          LK_jishi_flag=1;
                          LK_jishi=2000;
                        }
                ...
                lkcongzhongjiansaomiao();//图像处理中间扫描@@@@@@@@@@@@@@@@@

                  if(star_lineflag==1)
                        LK_yanshi++;    //变量用来停车时间把握 30*5ms

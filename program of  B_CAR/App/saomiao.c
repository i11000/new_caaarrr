...
#include "math.h"
int qianzhan=29;
int kp=220,kd=45,my_piancha,my_lastzhongjian=40,currentzhongjianzhi=40,rightheixian_flag=0,leftheixian_flag=0,xielv;
...
char jiji=0,papa=0,chuhuan_you=0,chuhuan_zuo=0,xielv_flag_count=0;

int yuan_diu=0,qishihang=0;
int chushizichuli=0,sw_zuo,sw_you;
extern char jieduan=0,jieduan_mark=0;
char street_width=0;
char street_len=0;
...
void lkcongzhongjiansaomiao()
{
    ...
    // *******************************************************十字处理*********************************************************//
      ...
      if(duandian<16) // *****************************十字
      {
         ...
         else if(shizishu>4&&rushizi<=10&&shangshizidiyidian>=20 && xielv_flag_count>15)//出十字
          {
               chushizi_flag=1;
               xielv_flag_count=0;;
               //jiji++;
  ...
  if(chushizi_flag==1)
  {
    ...
    chushizichuli=1;
      ...
      for(int i=55;i>0;i--)
      {
              if(imgyiwei[i][39]!=0 || imgyiwei[i][40]!=0)
              {
                  street_len++;
              }
              else
      ...
      if(yuan_diu == 1 && i == 35)    //右圆环
      {
        ...
        && street_len >46 && street_width>50 && !shizishu)
        {  

          jieduan = 0;
          jieduan_mark = 0;
          for(int ii = 35;ii>5;ii--){
            if(imgyiwei[ii][79]==0 && jieduan ==0){
              jieduan = 1;
              jieduan_mark = ii;
            }
            else if(imgyiwei[ii][79]!=0 && jieduan == 1){
              if(jieduan_mark - ii > 15 && jieduan_mark - ii < 25){
                jieduan = 2;
                jieduan_mark = ii;
              }
              else break;
            }
            else if(imgyiwei[ii][79]==0 && jieduan ==2){
              if(jieduan_mark - ii >2 && jieduan_mark - ii < 7){
                  cur_R_ready_flag = 1;                
                   break;
              }
              else break;
        ...
        if(cur_R_ready_flag||cur_R_diyi_flag) //交换第一段标志
        {
          ...
          &&shaobudaozuo_flag[i+2]==1 && street_len>43)
          {
...
//             shaobudaozuo_flag[24]==0
             cur_R_real_time_flag> 120 && street_len > 40
               )
          {
                       //   jiji++;
                  cur_R_real_rest_flag = 1;        //出圆环标志
...
//      if(i==21&&cur_R_real_rest_flag&&cur_R_real_time_flag1>40&&!Left_Add_Flag[i]&&!Left_Add_Flag[i+2]&&Left_Add_Line[i]>50&&Left_Add_Line[i+2]<60)
          ...
          if(yuan_diu == 1 && i == 35)
      {
        ...
        && street_len >46 && street_width>50  && !shizishu)
        {
          ...
          jieduan_mark = 0;
          for(int ii = 35;ii>5;ii--){
            if(imgyiwei[ii][0]==0 && jieduan ==0){
              jieduan = 1;
              jieduan_mark = ii;
            }
            else if(imgyiwei[ii][0]!=0 && jieduan == 1){
              if(jieduan_mark - ii > 15 && jieduan_mark - ii < 25){
                jieduan = 2;
                jieduan_mark = ii;
              }
            ...
            else if(imgyiwei[ii][0]==0 && jieduan ==2){
              if(jieduan_mark - ii >2 && jieduan_mark - ii < 7){
                cur_L_ready_flag = 1;
                   break;
              }
       ...
       //?
        if(cur_L_ready_rest_flag||cur_L_nei_flag) //交换内圆标志 第一段和内圆准备标志全清空
        {
          ...
          cur_L_nei_flag = 1;         
          //在的二段后0.4秒内:在35行36行处左边不丟线,右边丟线.赛道长度大于40.宽度大于50.35行左边赛道在[10,30]范围内.
          if(//shaobudaozuo_flag[i]==0 && shaobudaozuo_flag[i+2]==0 && shaobudaoyou_flag[i]==1
          //&& shaobudaoyou_flag[i+2]==1 && street_len>40 && street_width>50
//             shaobudaoyou_flag[i] ==1 && shaobudaoyou_flag[i+2] ==1 && shaobudaozuo_flag[i]==0 &&shaobudaozuo_flag[i+2]==0
//        && street_len >46 && street_width>50
      ...
      //             shaobudaoyou_flag[22]==0 && shaobudaoyou_flag[25]==0 && shaobudaoyou_flag[31]==0
                    cur_L_real_time_flag> 150 && street_len > 40
              ...
              {       
                cur_L_real_delay_flag = 0; //圆环结束，总标志清空   
              }         
...
/****************************圆环减速***************************************************/  
    ...
    if(guai_flag && guai_flag<=70)//电机减速反转的时间 5ms*30=150ms
    {
         guai_flag++;

         if(guai_flag&&guai_flag<=70)//轮子拐弯保持的时间
         {
...
void pianchachuli()//偏差处理
{
    ...
    /**********************************圆环偏差处理******************************/ 
    if(ruyuanhuan_flag>0 && ruyuanhuan_flag<30)//拐25ms 进左圆环
    {
          //jiji++;
          ruyuanhuan_flag++;
    ...
    if(ruyuanhuan_flag>0 && ruyuanhuan_flag<30)//拐25ms 进右圆环
    {
          //jiji++;
          ruyuanhuan_flag++;
        ...
        if(chuhuanyou_flag>0 && chuhuanyou_flag<30)//拐50ms
    {
          ...
          cur_R_ready_flag=0;         //右圆环预判断初始识别标志
          cur_R_ready_delay_flag=0;   //右圆环预判断弛懈标志
          ...
          cur_R_ready_time_flag=0;   //右圆环预判断计时变量
          cur_R_ready_rest_flag=0;    //右圆环预判断复位变量
          ...
          cur_R_real_time_flag=0;    //右圆环准确弛懈识别标志
          cur_R_real_time_flag1=0;   //右圆环准确弛懈识别标志1
    }
    else
    {
       chuhuanyou_flag=0;
   ...
   /*****************************出左环*********************************/
    if(chuhuanzuo_flag>0 && chuhuanzuo_flag<10)//拐50ms
    {
          //jiji++;
          chuhuanzuo_flag++;
          my_piancha-=60;
          cur_L_ready_flag=0;         //右圆环预判断初始识别标志
          cur_L_ready_delay_flag=0;   //右圆环预判断弛懈标志
          ...
          cur_L_ready_time_flag=0;   //右圆环预判断计时变量
          cur_L_ready_rest_flag=0;    //右圆环预判断复位变量

          /*圆环准确识别*/
          cur_L_real_flag=0;          //右圆环准确判断识别标志
          cur_L_real_delay_flag=0;    //右圆环准确弛懈识别标志
          cur_L_real_rest_flag=0;     //右圆环准确复位识别标志

          cur_L_real_into_flag=0;     //右圆环准确入环识别标志

          cur_L_real_CH_flag=0;

          cur_L_real_time_flag=0;    //右圆环准确弛懈识别标志
          cur_L_real_time_flag1=0;   //右圆环准确弛懈识别标志1
    }
    else
    {
       chuhuanzuo_flag=0;
